export const projects = [
  {
    title: 'Personal Portfolio',
    description: "A website built to learn more about React, and give potential employers the ability to have a more interactive skill and experience display while also demonstrating my value as a potential employee.",
      image: '/images/Portfolio.png',
      tags: ['React', 'JavaScript', 'Node', 'CSS', 'HTML'],
    visit: 'https://github.com/bledsoef/finn-bledsoe-react-portfolio',
    id: 0,
  },
  {
    title: 'Discogs Vinyl Randomizer',
    description:"Scrapes the Discogs website for a user's media collection and displays a random vinyl or CD selection or a genre specific selection.",
    image: '/images/Randomizer.png',
    tags: ['Python', 'Discogs API', 'PyQt6'],
    visit: 'https://github.com/bledsoef/random-vinyl-selector-discogs',
    id: 1,
  },
  {
    title: 'Ebay Price Comparison Application',
    description: "An application that takes a product and price searches ebay for cheaper prices, and displays those to the user.",
      image: '/images/3.jpg',
      tags: ['Python', 'PyQt6', 'BeatifulSoup', 'Requests'],
    visit: 'https://github.com/bledsoef/price-comparison-application',
    id: 2,
  },
  {
    title: 'Berea College Celts Application',
    description: "A full-stack application for the Berea College Celts department which is used by staff and faculty to managecourses and events for 1700 Berea College students, faculty, and staff using JavaScript, Python, HTML, and CSS.",
    image: '/images/4.jpg',
    tags: ['Python', 'JavaScript/JQuery', 'HTML', 'CSS', 'Flask'],
    visit: 'https://github.com/BCStudentSoftwareDevTeam/celts',
    id: 3,
  },
];

export const TimeLineData = [
  { year: 2017, text: 'Started my journey', },
  { year: 2018, text: 'Worked as a freelance developer', },
  { year: 2019, text: 'Founded JavaScript Mastery', },
  { year: 2020, text: 'Shared my projects with the world', },
  { year: 2021, text: 'Started my own platform', },
];


export const Accomplishments = [
  { number: 20, text: 'Open Source Projects'},
  { number: 1000, text: 'Students', },
  { number: 1900, text: 'Github Followers', },
  { number: 5000, text: 'Github Stars', }
];